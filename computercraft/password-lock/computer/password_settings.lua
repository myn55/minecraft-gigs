-- Author: Coronhaus
-- Auto-generated password settings by the installer

local module = {
    terminal = false; -- If this computer is solely made to be a password terminal outputting a redstone signal from a side
    redstoneSide = ''; -- If a terminal, the side for the redstone signal to go on
    redstoneStrength = 0; -- Strength of the redstone signal
    speakerInstalled = false; -- If a speaker is installed on any side
}

return module